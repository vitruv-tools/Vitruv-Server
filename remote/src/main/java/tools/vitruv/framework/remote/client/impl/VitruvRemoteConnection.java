...
public class VitruvRemoteConnection implements VitruvClient {
  ...
  private final JsonMapper mapper;
    ...
    /**
     * Starts an asynchronous change propagation for the given view with the given
     * changes.
     * 
     * @param uuid   The UUID of the view to propagate changes for.
     * @param change The changes to propagate.
     * @return A task ID that can be used to query the status of the async
     *         operation.
     * @throws BadServerResponseException If the server answered with a bad response
     *                                    or a connection error occurred.
     */
    String startAsyncPropagation(String uuid, VitruviusChange<?> change) throws BadServerResponseException {
        try {
            change.getEChanges().forEach(it -> {
                if (it instanceof InsertRootEObject<?>) {
                    ((InsertRootEObject<?>) it).setResource(null);
            ...
            var request = HttpRequest.newBuilder()
                    ...
                    .build();
            var response = sendRequest(request);
            return response.body(); // Returns the task ID
        } catch (IOException e) {
            throw new BadClientResponseException(e);
        }
    }
